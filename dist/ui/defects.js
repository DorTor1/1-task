...
router.get('/', async (req, res) => {
    ...
    const where = {};
    if (filters.status && Object.values(DefectStatus).includes(filters.status)) {
    ...
    const resolvedOrder = orderMap[filters.sort] ?? orderMap.createdAt_desc;
    const orderInput = [resolvedOrder];
...
router.post('/:id', requireRole(['MANAGER']), async (req, res) => {
    ...
    if (!parse.success)
        return res.status(400).redirect(`/defects/${req.params.id}/edit`);
    const defectId = req.params.id;
    ...
    if (old.title !== updated.title)
        histories.push({ defectId: updated.id, field: 'title', oldValue: old.title, newValue: updated.title, changedById: user.id });
    if ((old.description || '') !== (updated.description || ''))
        histories.push({ defectId: updated.id, field: 'description', oldValue: old.description || '', newValue: updated.description || '', changedById: user.id });
    if (old.priority !== updated.priority)
        histories.push({ defectId: updated.id, field: 'priority', oldValue: old.priority, newValue: updated.priority, changedById: user.id });
    if ((old.stageId || '') !== (updated.stageId || ''))
        histories.push({ defectId: updated.id, field: 'stageId', oldValue: old.stageId || '', newValue: updated.stageId || '', changedById: user.id });
    if ((old.assigneeId || '') !== (updated.assigneeId || ''))
        histories.push({ defectId: updated.id, field: 'assigneeId', oldValue: old.assigneeId || '', newValue: updated.assigneeId || '', changedById: user.id });
    for (const h of histories)
        await prisma.defectHistory.create({ data: h });
...
router.post('/:id/status', requireRole(['MANAGER', 'ENGINEER']), async (req, res) => {
    ...
    if (!parse.success)
        return res.status(400).redirect(`/defects/${req.params.id}`);
    ...
    if (!allowed.includes(target))
        return res.status(400).render('error', { title: 'Ошибка', message: 'Недопустимый переход статуса' });
...
router.post('/:id/comments', requireRole(['MANAGER', 'ENGINEER']), async (req, res) => {
    ...
    const defectId = req.params.id;
    if (!parse.success)
        return res.status(400).redirect(`/defects/${defectId}`);
    const user = req.session.user;
